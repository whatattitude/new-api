...
func GetSelf(c *gin.Context) {
	...
	// 计算累计充值金额（所有成功充值记录的 money 总和）
	var totalTopupAmount float64
	err = model.DB.Model(&model.TopUp{}).
		Where("user_id = ? AND status = ?", id, common.TopUpStatusSuccess).
		Select("COALESCE(SUM(money), 0)").
		Scan(&totalTopupAmount).Error
	if err != nil {
		// 如果查询失败，设置为0
		totalTopupAmount = 0
	}
	...
	responseData := map[string]interface{}{
		...
		"topup_multiplier":   user.TopupMultiplier,
		"total_bonus_amount": user.TotalBonusAmount,
		"total_topup_amount": totalTopupAmount, // 累计充值金额
		"group":              user.Group,
		"quota":              user.Quota,
